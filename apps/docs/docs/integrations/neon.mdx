...
async function advance(deliveryId: string, ref: DeliveryRef) {
  ...
  // The case keeps a terminal state, and RETURNING reads back the persisted state
  // even when a concurrent resume already finished it.
  const { rows } = await pool.query(
    `update deliveries
       set state = case when state in ('published', 'failed') then state else $2 end
     where id = $1
     returning state`,
    [deliveryId, outcome.state],
  );
  return { ...outcome, state: rows[0].state as DeliveryOutcome["state"] };
}
