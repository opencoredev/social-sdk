...
export async function queuePublish(request: PublishRequest, publishAt?: Date) {
  ...
  const delayMs = publishAt ? Math.max(0, publishAt.getTime() - Date.now()) : 0;
...
The `idempotencyKey` stops a retried send from queuing the publish twice. Restate keeps idempotency results for 24 hours by default, and the draft's own state rejects duplicates after that.
