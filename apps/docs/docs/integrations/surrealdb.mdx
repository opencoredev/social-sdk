...
DEFINE INDEX account_unique ON connected_account FIELDS tenant, platform, account_id UNIQUE;
...
export function surrealThreadsWorkflowStore(db: Surreal): ThreadsWorkflowStore {
  ...
  return {
    ...
    async claim(id) {
      const [rows] = await db
        .query<[unknown[]]>(
          surql`UPDATE ${record(id)} SET claimed_until = time::now() + 5m
    ...
    async release(id) {
      await db.query(surql`UPDATE ${record(id)} SET claimed_until = NONE`).collect();
    },
  };
}
```
