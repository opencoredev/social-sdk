...
- Use Zernio, Post for Me, or PostFast when the application will use that provider's connection and execution service.

Managed services are optional. A direct integration does not require a Zernio, Post for Me, PostFast, or Social SDK account. A managed integration uses the developer's own provider account and may create provider charges.
