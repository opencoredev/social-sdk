...
Connection attempts expire after ten minutes by default. Account selection must finish before expiry. The PKCE verifier and provider state stay in the store; `begin` returns neither. Keep them private in server storage, encrypt stored credentials separately, and use compare-and-set token revisions when refreshing.
...
**Default scopes:** `atproto` and `transition:generic`. **PKCE:** S256, sent in a
pushed authorization request with DPoP-bound tokens.

Bluesky uses `blueskyOAuth`, which follows the AT Protocol OAuth profile instead
of a fixed provider endpoint. Pass the handle, DID, or server URL the user typed
as `loginHint` to `begin`. There is no client secret: `clientId` is the URL of
your client metadata document, and confidential clients sign with an ES256
`clientKey`. The verified session goes to `sessionSink` rather than
`credentialSink`. Refresh tokens rotate.
...
See the [Bluesky page](/platforms/bluesky) for client metadata and the adapter transport.
