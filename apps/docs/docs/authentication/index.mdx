...
**Default scopes:** `openid`, profile, and `youtube.readonly`. **PKCE:** the
provider flow does not add PKCE.

Google refresh tokens use `refreshOAuthToken("youtube", ...)`. Request
`youtube.upload` when uploading videos.
...
**Default scopes:** `tweet.read`, `users.read`, and `offline.access`.
**PKCE:** S256.
...
**Default scopes:** `user.info.basic` and `video.publish`. **PKCE:** S256.
