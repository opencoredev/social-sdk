...
No. Social SDK is a library. It provides no hosted service, database, queue, credential proxy, scheduler worker, or billing layer. Direct integrations call platforms with your credentials; optional hosted platforms call Zernio, Post for Me, or PostFast with your own provider account.

## What does it cost?

The package is open source under the MIT license. Platforms and managed providers charge separately under your own accounts: X API plans, Meta app products, Zernio, Post for Me, or PostFast subscriptions. The SDK never pools keys or resells provider usage.
