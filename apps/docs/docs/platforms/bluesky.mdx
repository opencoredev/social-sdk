...
- The OAuth helper does not revoke tokens. If identity verification fails after a token exchange, the tokens are discarded without calling the revocation endpoint.
