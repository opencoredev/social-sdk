...
- If identity verification fails after a token exchange or refresh, the helper tries once to revoke the new tokens at the authorization server's `revocation_endpoint`, when the server advertises one. It revokes the refresh token, or the access token if no refresh token was issued. This is a single best-effort request with no retry. A failed revocation is ignored, and the original verification error is thrown. The helper does not revoke tokens in any other case, such as when an account disconnects.
- Handle resolution through `https://<handle>/.well-known/atproto-did` follows HTTP redirects, as the handle specification allows, but only to HTTPS URLs. OAuth metadata requests do not follow redirects.
