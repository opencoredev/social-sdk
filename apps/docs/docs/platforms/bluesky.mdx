...
The typed native module covers video upload, reposts and quote posts, deletion, graph relationships,
likes, actor search, lists, moderation reports, notification reads and seen state, profile
reads/updates, feed discovery, search, and `chat.bsky` conversation/message operations. Video and
chat responses remain provider-controlled processing or moderation states; persist native
