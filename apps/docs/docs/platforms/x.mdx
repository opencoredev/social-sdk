...
- Profile updates are not available. X API v2 has no endpoint for editing a profile, and the legacy v1.1 `account/update_profile` method is no longer in the X documentation. It also needs OAuth 1.0a, which this adapter does not use.
