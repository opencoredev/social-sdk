...
const social = createSocial({
  backend: x({
    ...
    // Required for full-archive search and filtered stream.
...
latest event for each distinct `dm_conversation_id` in that page, and preserves X's pagination
cursor. `listMessages` reads `GET /2/dm_conversations/{dm_conversation_id}/dm_events`, and `send`
...
The typed native module also exposes user lookup, followers and following, follow and unfollow, likes, mute controls, blocked and muted lists, lists, user and home timelines, mentions, retweet undo, direct-message send and reads, video and GIF uploads, polls, quotes, deletion, bookmarks, and filtered streams. Paginated methods return the SDK `Page` shape, so pass `nextCursor` back as `cursor`.

Full-archive search (`searchAllPosts`) and filtered streams require an app-only bearer token. Configure `appBearerToken`; the adapter raises a `missing_permission` `SocialError` before making a request when it is absent. User-scoped methods continue to use the configured OAuth user token.
