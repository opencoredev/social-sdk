...
Pass the app secret as `webhookSecret` to verify `X-Hub-Signature-256` through `adapter.webhooks.verify`, and answer the GET verification with `answerMetaWebhookChallenge`. Meta's Threads documentation does not say whether the Threads app secret or the Meta app secret signs deliveries, so confirm with a real delivery. Replies decode to `comment.received` and deletions to `post.removed`; mentions and publish events stay `unknown`. See [Process webhooks](/events).
...
- Published posts cannot be edited. The [Threads posts documentation](https://developers.facebook.com/docs/threads/posts) sets text and attachments when the container is created and has no update endpoint, so `posts.update` is declared `unsupported-by-platform`. Delete and publish again if the content must change.
