...
description: List, reply to, and moderate supported comments and messages.
---
...
`listConversations` derives one latest event per conversation from the current
`/2/dm_events` page, so it is not a complete historical inbox.
