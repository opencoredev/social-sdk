...
| ----------------------------------- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `posts.cancelScheduled(job)`        | Account-scoped `ScheduledJobRef` from a scheduled outcome | Zernio deletes the future scheduled record. Post for Me retains the record as a draft, preserving its timestamp and content. PostFast and Postiz delete the future scheduled record. Direct YouTube clears `publishAt` and keeps the video private. |
| `posts.deleteBackendRecord(record)` | `BackendPostRef` with `recordId`                          | Deletes a draft provider record on Zernio or Post for Me, a failed record on PostFast, or a failed or draft record on Postiz. Scheduled and dispatched records are rejected.                                                                        |
