...
| --------- | ---------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X         | `not-implemented-by-adapter` | [`POST /2/tweets`](https://docs.x.com/x-api/posts/create-post) has no publish-time field. X schedules posts only through the [Ads API `scheduled_tweets` endpoint](https://docs.x.com/x-ads-api/creatives), which needs Ads API access, an ads account and OAuth 1.0a. |
...
These sources were checked on 2026-09-24. Because none of these platforms holds a scheduled post, the direct adapters do not implement `posts.cancelScheduled`. To publish later on these platforms, run the publish from your own durable job runner, or use a hosted backend that schedules on your behalf.
